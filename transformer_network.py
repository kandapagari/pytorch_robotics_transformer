...
# here[https://github.com/google-research/robotics_transformer].
...
class TransformerNetwork(nn.Module):
    ...
    def __init__(
            ...
            input_tensor_space: spaces.Dict,
            # action space like dict. keys are world_vector, rotation_delta,
            # gripper_closedness_action, terminate_episode
            output_tensor_space: spaces.Dict,
            train_step_counter: int = 0,
            # Dimensionality of tokens from the output layer. This is also dimensionality of
            # tokens from the input layer.
            vocab_size: int = 256,
            # RT1ImageTokenizer outputs(=context_image_tokens) has embedding dimension of
            # token_embedding_size. This will finally be utilized in 1x1 Conv in
            # EfficientNetEncoder class.
            token_embedding_size: int = 512,
            num_layers: int = 1,
            # This corresponds to key_dim which is the size of each attention head for query, key
            # and values.
            layer_size: int = 4096,
            num_heads: int = 8,
            # This corresponds to d_model which is embedding dimension of each token in
            # transformer part.
        ...
        self._action_token_emb = nn.Linear(
            self._vocab_size, self._token_embedding_size)
        ...
        self._state_space = spaces.Dict(
            {
                ...
                spaces.Box(low=-np.inf, high=np.inf,
                           shape=(
                               time_sequence_length, self._tokens_per_context_image,
                ...
                # Our data is like context_image_tokens + action_tokens + context_image_tokens +
                # action_tokens + context_image_tokens ...
                # 1 time step means [context_image_tokens + action_tokens]
                # seq_idx means which time steps we are. But it is adjusted to
                # time_sequence_length when it exceeds time_sequence_length.
            }
        )
    ...
    # _action_tokens_mask is for loss computing. This has all indexes of action tokens in all
    # tokens.
    # We can know which output tokens are action predictions by _action_tokens_mask - 1.
    # _default_attention_mask is modified causal mask because we will only use observations
    # tokens when predicting actions.
    ...
    def _generate_masks(self):
        ...
        self._all_num_tokens = self._time_sequence_length * \
            self._single_time_step_num_tokens
        ...
        self._action_tokens_mask.extend(
            x + n + self._tokens_per_context_image
            for n, x in itertools.product(
                range(0, self._all_num_tokens,
                      self._single_time_step_num_tokens),
                range(self._tokens_per_action),
            )
        )
        # The look ahead mask ensures causality.
        # This is a lower triangular matrix. All elements other than 0 are 1.
        # 0 means mask.
        self._default_attention_mask = torch.tril(torch.ones(
            (self._all_num_tokens, self._all_num_tokens), dtype=torch.uint8))
    ...
    def forward(self,
        ...
        if outer_rank == 1:  # This is an inference call
            ...
            # This index means the output of the last observation token that is at action_t
            # time step.
            start_index = (
            ...
            for k in range(self._tokens_per_action):
                ...
                # replace action_tokens[:, action_start_index] with the predicted token.
                # Note that this is not insert.
            ...
            predicted_tokens_for_output = torch.concat(
                current_action_tokens, 1)  # [1, self._tokens_per_action]
            one_state_action_tokens = predicted_tokens_for_output.unsqueeze(
                1)  # [1, 1, self._tokens_per_action]
            ...
            # replace state_action_tokens[:, action_t, ...] with the predicted tokens.
            # Note that this is not insert.
            network_state['action_tokens'] = torch.concat([
                state_action_tokens[:,
                                    :action_t, ...], one_state_action_tokens,
            ...
            # Gather all predicted actions for the action loss. Use fancy index to extract
            # all predicted actions.
            predicted_action_index = torch.tensor(self._action_tokens_mask) - 1
            # (bs, t*tokens_per_action, vocab_size)
            action_logits = output_tokens[:, predicted_action_index]
            ...
            predicted_tokens_for_output = torch.argmax(
                action_logits_for_output, dim=-1)
            ...
            action_loss = torch.mean(
                self._loss_object(action_logits_for_training.permute(
                    0, 3, 1, 2), action_tokens) / num_items,  # (b, t, self._tokens_per_action)
                dim=-1)
    ...
    def _get_outer_rank(self, observations: Dict[str, torch.Tensor]) -> int:
        ...
        for k, obs_value in observations.items():
    ...
    def _transformer_call(
        ...
        input_token_sequence = self._assemble_input_token_sequence(
            context_image_tokens, action_tokens, batch_size)  # [b, t*num_tokens, emb_dim]
        # run transformer
        output_tokens, self._attention_scores = self._transformer(
            input_token_sequence, attention_mask)  # (bs, t*num_tokens, vocab_size)
    ...
    def _assemble_input_token_sequence(self, context_image_tokens, action_tokens, batch_size):
        # embed action tokens
        action_tokens = F.one_hot(
            action_tokens, num_classes=self._vocab_size).to(torch.float32)
        # [b, t , num_action_tokens, emb_dim]
        action_tokens = self._action_token_emb(action_tokens)

        # This removes autoregressive conditioning on actions because it did not benefit
        # performance and slowed inference.
        ...
        input_token_sequence = input_token_sequence.view(
            batch_size, -1, self._token_embedding_size)  # [b, t*num_tokens, emb_dim]
    ...
    def _tokenize_images(self, observations, network_state):
        ...
        context = self._extract_context_from_observation(
            observations, input_t)  # [b, t, emb-size] or None
        ...
        context_image_tokens = self._image_tokenizer(
            image, context=context)  # (batch, t, num_tokens, embedding_dim)
        ...
        # Here, we combine past context_image_tokens of network_state with current
        # context_image_tokens.
        # network_state only store tokens within time_sequence_length time steps.
        # This means network_state does not store the tokens for all past steps, but only for
        # time_sequence_length time steps.
        # if current time step >= time_sequence_length, we store context_image_tokens after we
        # discard the oldest context_image_tokens.
        ...
        if outer_rank == 1:  # This is an inference call
            ...
            # if seq_idx == time_sequence_length, state_image_tokens will be shifted to the left
            # along time axis
            # seq_idx will be incremented in forward function. But it is adjusted so that it never
            # exceed time_sequence_length.
            ...
            context_image_tokens = torch.concat([
                # Note that in inference, size of context_image_tokens is (batch, 1, num_tokens,
                # embedding_dim)
    ...
    def _tokenize_actions(self, observations, network_state):
        ...
        if outer_rank == 1:  # This is an inference call
            ...
            action_tokens = torch.roll(
                action_tokens, -1, 1) if seq_idx == self._time_sequence_length else action_tokens
        else:
            assert outer_rank == 2
    ...
    def _extract_context_from_observation(self, observations, seq_len):
        ...
        if 'natural_language_embedding' in observations:
            ...
            context = observations['natural_language_embedding']
            if outer_rank == 1:
