...
class FilmConditioning(nn.Module):
    ...
    # conditioning: text which corresponds to x in FiLM paper. this is one vector that
    # is created from a text,
    # note that this is not embedding vectors from a text.
    # Please refer to Universal Sentence Encoder. (B, D). D = 512.
    def forward(self, conv_filters: torch.Tensor, conditioning: torch.Tensor):
        projected_cond_add = self._projection_add(
            conditioning)  # (B, D) -> (B, C)
        projected_cond_mult = self._projection_mult(conditioning)

        projected_cond_add = projected_cond_add.unsqueeze(
            2).unsqueeze(3)  # (B, C) -> (B, C, 1, 1)
        projected_cond_mult = projected_cond_mult.unsqueeze(2).unsqueeze(3)

        return (1 + projected_cond_mult) * conv_filters + projected_cond_add
