...
def convert_dtype_and_crop_images(images: torch.Tensor, ratio: float = 0.07):
    ...
    shift_h = torch.randint(0, 2 * ud_pad + 1, size=[])
    shift_w = torch.randint(0, 2 * lr_pad + 1, size=[])

    grid_h, grid_w = torch.meshgrid(torch.arange(shift_h, shift_h + height),
                                    torch.arange(shift_w, shift_w + width),
                                    indexing='ij')
...
if __name__ == '__main__':
    ...
    images = np.tile(np.expand_dims(images, 0),
                     (10, 1, 1, 1))  # batch size: 10
    images = torch.from_numpy(images).permute(
        0, 3, 1, 2)  # (b, h, w, c) -> (b, c, h, w)
