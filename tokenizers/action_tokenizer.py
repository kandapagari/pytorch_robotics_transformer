...
action_space = gym.spaces.Dict(
    {
        ...
        'rotation_delta': gym.spaces.Box(low= -np.pi / 2  , high= np.pi / 2, shape=(3,),
        dtype=np.float32),
        'gripper_closedness_action': gym.spaces.Box(low= -1.0  , high= 1.0, shape=(1,),
        dtype=np.float32)
    }
)
or 
action_space = gym.spaces.Dict(
            OrderedDict([
                ('terminate', gym.spaces.Discrete(2)), 
                ('world_vector', gym.spaces.Box(low= -1.0, high= 1.0, shape=(3,),
                dtype=np.float32)),
                ('rotation_delta', gym.spaces.Box(low= -np.pi / 2., high= np.pi / 2., shape=(3,),
                dtype=np.float32)),
                ('gripper_closedness_action', gym.spaces.Box(low= -1.0  , high= 1.0, shape=(1,),
...
These information are very convenient when interpreting, examining, clipping, and processing the
action
...
class RT1ActionTokenizer():
    def __init__(self,
        ...
        for action in self._action_order:
            ...
            # If a action is spaces.Box, get the number of tokens using shape.
                ...
                raise ValueError(
                    'We assume action_space is defined by either\
                        gym.spaces.Discrete or gym.spaces.Box')
    ...
    def tokenize(self, action: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        # If a is Discrete, the size of a is () or (batch,), which means the former is scalar of
        # Tensor type and the later is 1-d Tensor.
    ...
    def detokenize(self, action_tokens: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        for k in self._action_order:
            ...
            if isinstance(space, spaces.Discrete):
                # The size of action_tokens[k] : (1,) or (batch,), which means the former is
                # scalar of Tensor type and the later is 1-d Tensor.
